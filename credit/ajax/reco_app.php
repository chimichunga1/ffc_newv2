<?php
	...
	$columns = array(
        ...
        {
			global $con;
			$bwu_files=$con->myQuery("SELECT * FROM bwu_files WHERE is_deleted='0' AND loan_id={$d}")->fetchAll(PDO::FETCH_ASSOC);
			...
			$action_buttons.="<input type='hidden' name='type' value='submit_mark2'>";
            $action_buttons.="<button type='submit' class='btn btn-success' data-toggle='tooltip' data-placement='top' name='action' title='Approve Request'><i class='fa fa-check'></i></button> ";
			$action_buttons.="</form>";
			}
			...
			$action_buttons.="<form method='post' action='../move_loan.php' style='display: inline' onsubmit='return confirm(\"Approve This Request?\")'>";
            $action_buttons.="<input type='hidden' name='id' value={$d}>";
            $action_buttons.="<input type='hidden' name='type' value='submit_mark_redo'>";
            $action_buttons.="<button type='submit' class='btn btn-danger' data-toggle='tooltip' data-placement='top' name='action' title='Approve Request'><i class='fa fa-rotate-left'></i></button> ";
			$action_buttons.="</form>";
			// $action_buttons.= " <a href='word.php?id={$d}'><button type='submit' class='btn bg-gray' id='btn-print' name='btnprint' data-toggle='tooltip' data-placement='top' title='Print'> <i class='fa fa-print'> </i></button></a> ";
	...
	require( '../../support/ssp.class.php' );
		...
		$filter_sql="";
		$whereAll="ll.is_deleted=0 AND ll.loan_status_id='4'  AND (ll.ci_check_by='0' OR ll.ci_check_by='$user_id')" ;
		$whereAll.=$filter_sql;
