...
require_once('../../support/config.php');
 ...
 $con->myQuery("INSERT INTO  cheque_voucher(cv_no,clnt_id,details) VALUES(?,?,?) ",array($string1[0],$string1[1],$string1[2]));

	 $idcv=$con->myQuery("SELECT MAX(cv_id) FROM cheque_voucher ")->fetch(PDO::FETCH_NUM);
			...
			for ($i=0; $i<count($string2)/3; $i++) { 
					...
					$array = '';
					$value='';
					$query = ' INSERT INTO cheque_dbcr(cv_v_id,cv_no,acc_code,debit_amount,credit_amount,clnt_id';
					$array[] = $idcv[0];
					$array[] = $string1[0];
					$array[] .= $a1;
					$array[] .= $b1;
