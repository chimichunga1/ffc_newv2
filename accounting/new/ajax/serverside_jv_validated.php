	...
	array( 'db' => 'fullname','dt' => ++$index,'formatter'=>function($d,$row)
		...
		array( 'db' => 'jv_id','dt' => ++$index,'formatter'=>function($d,$row)
		{
			 ...
			 $action_buttons="";
  			...
  			if($row['isValidated']=='0')
  			{
          	$action_buttons.='	  <a href="jv_gl_valid.php?id='.$d.'" ><button type="button" id="buttonremarks" class="btn btn-success"  data-toggle="tooltip" data-placement="top" title="Validate"><i class="fa fa-check"> </i></button></a>';
          	}
          	

          		$action_buttons.='   
			      <a href="jv_gl_edit.php?id='.$d.'" ><button type="button" id="buttonremarks" class="btn btn-warning"  data-toggle="tooltip" data-placement="top" title="Edit"><i class="fa fa-edit"> </i></button></a> ';
...
// var_dump($bindings);
$complete_query="SELECT CONCAT(cl.fname ,' ', cl.lname ) fullname ,j.jv_id, j.jv_no ,j.clnt_id,j.isValidated
