...
$queryjv=$con->myQuery("SELECT j.clnt_id  FROM `journal_voucher` j  WHERE j.`jv_id`='".$d."' and j.isDeleted='0'  ");
                    $queryjvfetch=$queryjv->fetch(PDO::FETCH_NUM);
...
$query2=$con->myQuery("SELECT c.`cv_id` FROM `cheque_voucher` c INNER JOIN client_list cl ON c.clnt_id=cl.client_number WHERE c.`clnt_id`='".$queryjvfetch[0]."' and c.isDeleted='0'  ");
                    while ($row2=$query2->fetch(PDO::FETCH_NUM)) 
                      
                    {


$query1=$con->myQuery("SELECT c.`cntrct_id`,c.`cd`,c.`acc_code`,c.`debit_amount`,c.`credit_amount`,c.`clnt_id`,CONCAT(cl.fname,' ',cl.lname ) fullname FROM `cheque_dbcr` c INNER JOIN client_list cl ON c.clnt_id=cl.client_number WHERE c.`cv_id`='".$row2[0]."' and c.isDeleted='0'  ");
