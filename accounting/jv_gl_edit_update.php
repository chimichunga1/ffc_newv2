...
$masterquery="SELECT j.`jv_id`,j.`acc_no`,j.`cd`,j.`acc_code`,j.`debit_amount`,j.`credit_amount`,j.`clnt_id`,CONCAT(cl.fname,' ',cl.lname ) fullname FROM `journal_dbcr` j INNER JOIN client_list cl ON j.clnt_id=cl.client_number WHERE j.`jv_no`='".$d."' and j.isDeleted='0' ";
