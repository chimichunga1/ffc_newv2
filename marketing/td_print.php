...
$client =  $con->myquery("SELECT * FROM instruction_sheet_td WHERE id = ?",array($_POST['tbl_id']))->fetch(PDO::FETCH_ASSOC);
    if(count($client) <= 0){
    redirect('instruction_sheet_prep.php');
    Alert('User not found','warning');
    }
...
$isType = $con->myQuery("SELECT B.name AS isType FROM loan_list A JOIN loan_approval_type B ON A.loan_type_id = B.id WHERE A.id = ? AND A.is_deleted =0",array($client['ll_id']))->fetch(PDO::FETCH_ASSOC);   
...
while($row = $collat->fetch(PDO::FETCH_ASSOC)){
    $collatOut[$i]['facility'] = $row['unit_description'];
    $collatOut[$i]['engineno'] = $row['location_motor'];
    $collatOut[$i]['chassisno'] = $row['tct_no'];
    $collatOut[$i]['plateno'] = $row['tct_no'];
    $collatOut[$i]['lcpav'] = !empty($row['approve_value'])?number_format($row['approve_value'],2,'.',','):'';
    $collatOut[$i]['ltooff'] = $row['lto_agency'];
    $collatOut[$i]['ltocr'] = $row['cr_no'];
    $collatOut[$i]['ltoor'] = $row['or_no'];
    $collatOut[$i]['date'] = dateCon($row['collat_created_date']);
    $collatOut[$i]['insco'] = $row['insurance_comp'];
    $collatOut[$i]['policy'] = $row['policy_no'];
    $collatOut[$i]['expdate'] = dateCon($row['exp_date']);
    $collatOut[$i]['tuacc'] = "";
    $collatOut[$i]['unit'] = "";
    $collatOut[$i]['engineno_'] = "";
    $collatOut[$i]['chassisno_'] = "";
...
$pdf->Cell(196,5,'INSTRUCTION SHEET ('.$isType['isType'].')',0,1,"C");
$pdf->SetFont('Arial','B',8);
...
$pdf->Cell(30,4,'Amount of Line',0,0,'L');
$pdf->Cell(5,4,':',0,0,'L');
$pdf->Cell(45,4,isEmptyFloat($client['amount_line']),0,0,'L');
$pdf->Cell(36,4,'',0,0);
$pdf->Cell(45,4,'Available Balance',0,0,'R');
$pdf->Cell(5,4,':',0,0,'R');
$pdf->Cell(30,4,isEmptyFloat($client['avail_bal']),0,1,'R');

$pdf->Cell(30,4,'Date Approved',0,0,'L');
$pdf->Cell(5,4,':',0,0,'L');
$pdf->Cell(45,4,isEmptyDate($client['date_approved']),0,0,'L');
$pdf->Cell(36,4,'',0,0);
$pdf->Cell(45,4,'Propposed Availment',0,0,'R');
$pdf->Cell(5,4,':',0,0,'R');
$pdf->Cell(30,4,isEmptyFloat($client['prop_avail']),0,1,'R');

$pdf->Cell(30,4,'Outstanding Avail',0,0,'L');
$pdf->Cell(5,4,':',0,0,'L');
$pdf->Cell(45,4,isEmptyFloat($client['outstanding_avail']),0,1,'L');
...
$pdf->Cell(49,5,$client['term'].'  days',$right,0);
$pdf->Cell(40,5,'MAXIMUM TERM',$left,0);
$pdf->Cell(9,5,':',$colon,0,'C');
$pdf->Cell(49,5,$client['max_term'].' days',$right,1);

$pdf->Cell(40,5,'INTEREST RATE',$left,0);
$pdf->Cell(9,5,':',$colon,0,'C');
$pdf->Cell(49,5,convert($client['int_rate'])." %",$right,0);
...
$pdf->Cell(40,5,'DISCOUNT',$left,0);
$pdf->Cell(9,5,':',$colon,0,'C');
$pdf->Cell(49,5,convert($client['discount']),$right,0);
$pdf->Cell(40,5,'MANNER OF PAYMENT',$left,0);
$pdf->Cell(9,5,':',$colon,0,'C');
$pdf->Cell(49,5,$mp['name'],$right,1);

$pdf->Cell(98,5,'---------------------------------------------------',$border,0,'R');
$pdf->Cell(98,5,'',$border,1,'R');

$pdf->Cell(40,5,'NET PROCEEDS',$left,0);
$pdf->Cell(9,5,':',$colon,0,'C');
$pdf->Cell(49,5,convert($client['net_proceeds']),$right,0);
$pdf->Cell(98,5,'',$border,1,'R');


$pdf->Cell(196,7,'============================================================================================================================',0,1,'C');

$pdf->Cell(98,5,'NET PROCEEDS',1,0);
$pdf->Cell(98,5,convert($client['net_proceeds']),1,1,'R');
