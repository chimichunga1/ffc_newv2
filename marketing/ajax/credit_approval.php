<?php
	...
	$columns = array(
        ...
        {
			global $con;
			$bwu_files=$con->myQuery("SELECT * FROM bwu_files WHERE loan_id=?",array($row['id']))->fetch(PDO::FETCH_ASSOC);
            $action_buttons="";

			if($row['loan_status_id']=='5'){
				...
				$action_buttons.="<input type='hidden' name='type' value='submit_credit'>";
				$action_buttons.="<button type='submit' class='btn btn-success' data-toggle='tooltip' data-placement='left' name='action' title='Approv Request'><i class='fa fa-check'></i></button> ";
				$action_buttons.="</form>";
				...
				// $action_buttons.="<input type='hidden' name='emp_id' value={$row['employee_id']}>";
				if(!empty($bwu_files)){
				$action_buttons.="<a href='../download_file.php?id={$bwu_files['id']}&type=bwu' data-toggle='tooltip' data-placement='left' title='Download BWU' class='btn btn-default'><span class='fa fa-download'></span></a> ";
				}
				...
				$action_buttons.="<input type='hidden' name='id' value={$row['id']}>";
				$action_buttons.="<input type='hidden' name='type' value='submit_credit1'>";
	...
	require( '../../support/ssp.class.php' );
		...
		$filter_sql="";
		$whereAll="ll.is_deleted=0 AND ll.loan_status_id='5'" ;
		$whereAll.=$filter_sql;
