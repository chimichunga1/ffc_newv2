...
<section class="content-header">
	<?php
        Alert();
        $link = "collateral_form.php";
        $btnText = "Collateral Form";
        $mlGet = "";
		if(isset($_GET['ml'])){
            $link = "collateral_master_list.php";
            $btnText = "Collateral Master List";
            $mlGet = "&ml";
        }
	?>
	<div class="box">
	...
	<button type='button' class="btn btn-flat btn-default"><span class="fa fa-arrow-left"></span> <?php echo $btnText; ?> </button></a><br><br>
	<hr><br>
			<div class="row">
			...
			<?php if($_SESSION[WEBAPP]['user']['user_type_id'] <> 4): ?>		
				<a href='collateral_entry_form.php?id=<?=$_GET['id'].$mlGet;?>'><button type="submit" class="btn btn-flat btn-primary" id="btn-add" name="btnadd" style="float:right;"><i class="fa fa-plus"> &nbsp; Add Collateral</i>
				</button></a>
			<?php endif; ?>
...
<script type="text/javascript">
	...
	function edit(id){
		...
		//window.location ="/journal_entry.php?id=" + id;
		var href = window.location.href;
		var string = href.substr(0,href.lastIndexOf('/'))+"/collateral_list.php?id=" + id;
		window.location=string;
	}
    function editCollat(id,id1){
	
    //window.location ="/journal_entry.php?id=" + id;
    var href = window.location.href;
    var string = href.substr(0,href.lastIndexOf('/'))+"/collateral_entry_form.php?id=" + id+"&edit="+id1+"<?php echo $mlGet; ?>";
    window.location=string;
}
  ...
  var dttable="";
      $(document).ready(function() {
        dttable=$('#dataTables').DataTable({
                ...
                "select":true,
                "ajax":{
                    "url":"ajax/collateral_list.php?id=<?php echo $_GET['id'].$mlGet;?>"
